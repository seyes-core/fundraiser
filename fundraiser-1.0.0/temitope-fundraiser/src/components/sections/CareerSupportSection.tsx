...
import { Briefcase, BookOpen, Award, Users, Lightbulb, Network } from 'lucide-react';
...
export default function CareerSupportSection() {
  ...
  return (
    <>
        ...
        {SUPPORT_TYPES.map(({ type, icon: Icon, title, desc, cta }) => (
          <div key={type} className="card flex flex-col">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h3 className="font-semibold text-ink text-sm leading-tight">{title}</h3>
            </div>
            <p className="text-stone-500 text-sm leading-relaxed flex-1 mb-4">{desc}</p>
            <button
      ...
      {activeType && (
        <CareerSupportModal type={activeType} onClose={() => setActiveType(null)} />
      )}
    </>
  );
}
