    ...
    return (
      <div className="card text-center py-10">
        ...
        <p className="text-stone-500 max-w-sm mx-auto text-sm leading-relaxed">
          Your donation of{' '}
          <strong className="text-ink">{formatNaira(selectedAmount || 0)}</strong> has been
          received. You are helping me build my future in technology. 💙
        </p>
        {details.email && (
          <p className="text-xs text-stone-400 mt-3">A receipt has been sent to {details.email}</p>
        )}
        <button
          ...
          onClick={() => {
            ...
            setAmount(null);
            setCustomAmount('');
            setDetails({ email: '', twitter: '', linkedin: '', discord: '', anonymous: true });
          }}
        >
      ...
      <div className="card">
        ...
        <div className="space-y-3">
          ...
          <input
            ...
            onChange={e => setDetails(d => ({ ...d, twitter: e.target.value }))}
          />
          <input
            type="text"
            className="input-field"
          ...
          <input
            ...
            onChange={e => setDetails(d => ({ ...d, discord: e.target.value }))}
          />
          ...
          <label className="flex items-start gap-3 cursor-pointer pt-1">
            <input
              ...
              onChange={e => setDetails(d => ({ ...d, anonymous: e.target.checked }))}
              className="mt-0.5 accent-ink"
            />
            <span className="text-sm text-stone-600">
              Keep my donation anonymous (your name will not be publicly displayed)
            </span>
          </label>
        </div>
        ...
        <div className="flex items-center gap-2 mt-4 p-3 rounded-lg bg-mist text-xs text-stone-500">
          ...
          Payment processed securely by Flutterwave. Your details are never shared publicly.
        </div>

        <button className="btn-primary w-full mt-4 py-3.5" onClick={handleDonate}>
