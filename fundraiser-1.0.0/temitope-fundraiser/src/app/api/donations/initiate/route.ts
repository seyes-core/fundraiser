...
export async function POST(request: NextRequest) {
  ...
  const { amount, email, twitter_profile, linkedin_profile, discord_username, anonymous } = parsed.data;
  ...
  if (error) {
    ...
    return NextResponse.json({ success: false, error: 'Failed to initialize donation' }, { status: 500 });
  }
