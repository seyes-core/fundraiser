...
export default function AdminPage() {
  ...
  const saveStats = async () => {
    ...
    await fetch("/api/admin/stats", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: stats.id, ...statsEdit }),
    });
  ...
  const updateSubmissionStatus = async (id: string, status: string) => {
    ...
    setSubmissions((s) =>
      s.map((sub) =>
        sub.id === id
          ? { ...sub, status: status as CareerSupportSubmission["status"] }
          : sub,
      ),
    );
  ...
  const S = {
    ...
    badge: (c: string) =>
      ({
        ...
        fontWeight: 700,
        background:
          c === "new" ? "#EEF3F8" : c === "reviewed" ? "#FDF4E4" : "#EAF5EE",
        color:
          c === "new" ? "#1B3A5C" : c === "reviewed" ? "#8B5E0A" : "#2D6A4F",
      }) as React.CSSProperties,
  };
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: 13, color: "#6B6860", marginBottom: 28 }}>
          ...
          <button
            style={{ ...S.btn(), width: "100%", padding: "12px 0" }}
            onClick={handleLogin}
    ...
    <div style={S.page}>
      ...
      <div style={S.header}>
        <div>
          ...
          <div style={{ fontSize: 11, color: "#A8C4E0", marginTop: 2 }}>
            Temitope Fundraising Campaign
          </div>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          {msg && (
            <div style={{ fontSize: 12, color: "#A8E6C8", fontWeight: 600 }}>
              {msg}
            </div>
          )}
          <button
        ...
        >
          {(
            ...
            <button key={t} style={S.tab(tab === t)} onClick={() => setTab(t)}>
              ...
              {t === "guestbook" &&
                guestbook.filter((e) => !e.approved).length > 0 && (
                  <span
                    style={{
                      marginLeft: 6,
                      background: "#C8861A",
          ...
          <>
            ...
            >
              {[
                ...
                {
                  label: "Remaining",
                  value: formatNaira(stats.goal_amount - stats.amount_raised),
                },
              ...
              >
                <div>
                  ...
                  <input
                    type="number"
                    style={S.input}
                    value={statsEdit.amount_raised}
                    onChange={(e) =>
                      setStatsEdit((s) => ({
                        ...s,
                        amount_raised: parseInt(e.target.value) || 0,
                      }))
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    style={S.input}
                    value={statsEdit.donor_count}
                    onChange={(e) =>
                      setStatsEdit((s) => ({
                        ...s,
                        donor_count: parseInt(e.target.value) || 0,
                      }))
                    }
                  />
                </div>
            ...
            >
              ...
              >
                ...
                <div
                  style={{ fontWeight: 700, fontSize: 14, color: "#1A1917" }}
                >
                  {guestbook.filter((e) => !e.approved).length} pending
                </div>
                <div style={{ fontSize: 12, color: "#9C9A95" }}>
                  guestbook messages
                </div>
              </button>
              <button
        ...
        {tab === "guestbook" && (
          <>
            ...
            >
              ...
              <div style={{ fontSize: 12, color: "#9C9A95" }}>
                {guestbook.length} total ·{" "}
                {guestbook.filter((e) => !e.approved).length} pending
              </div>
            </div>
            {guestbook.length === 0 ? (
              <div style={{ ...S.card, textAlign: "center", color: "#9C9A95" }}>
                No messages yet.
              </div>
            ) : (
              guestbook.map((e) => (
                <div
                  ...
                  style={{
                    ...S.card,
                    borderLeft: `3px solid ${e.approved ? "#2D6A4F" : "#C8861A"}`,
                  }}
                >
                  <div
                    style={{
                    ...
                    >
                      ...
                      <span
                        style={{
                          ...
                          fontSize: 11,
                          fontWeight: 700,
                          background: e.approved ? "#EAF5EE" : "#FDF4E4",
                          color: e.approved ? "#2D6A4F" : "#8B5E0A",
                        }}
                      >
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <button
                      style={S.btn(e.approved ? "#6B6860" : "#2D6A4F")}
                      onClick={() => approveEntry(e.id, !e.approved)}
                    >
          ...
          <>
            <div style={{ ...S.card, marginBottom: 24 }}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <input
                  style={S.input}
                  placeholder="e.g. Laptop received!"
                  value={newUpdate.title}
                  onChange={(e) =>
                    setNewUpdate((u) => ({ ...u, title: e.target.value }))
                  }
                />
              </div>
              <div style={{ marginBottom: 16 }}>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setNewUpdate((u) => ({ ...u, content: e.target.value }))
                  }
                />
              </div>
              <button
                style={S.btn()}
                onClick={postUpdate}
                disabled={saving || !newUpdate.title || !newUpdate.content}
              >
            ...
            {updates.length === 0 ? (
              <div style={{ ...S.card, color: "#9C9A95", textAlign: "center" }}>
              ...
              updates.map((u) => (
                <div key={u.id} style={S.card}>
                  <div
                    style={{ fontSize: 11, color: "#9C9A95", marginBottom: 4 }}
                  >
                  ...
                  <div
                    style={{ fontSize: 13, color: "#6B6860", lineHeight: 1.75 }}
                  >
                    {u.content}
                  </div>
                </div>
                ...
                <div key={s.id} style={S.card}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    {s.status !== "actioned" && (
                      <button
                        style={S.btn("#2D6A4F")}
                        onClick={() => updateSubmissionStatus(s.id, "actioned")}
                      >
                        Mark Actioned
                      </button>
                    )}
        ...
        {tab === "donations" && (
          <>
            ...
            >
              ...
              <button style={S.btn("#2D6A4F")} onClick={exportCSV}>
                ⬇ Export CSV
              </button>
            </div>
            {donations.length === 0 ? (
                ...
                >
                  <thead>
                    <tr style={{ background: "#F3F2EF" }}>
                      {["Date", "Amount", "Status", "Email", "TX Ref"].map(
                        (h) => (
                          <th
                            key={h}
                            style={{
                  ...
                  <tbody>
                    {donations.map((d: Record<string, unknown>) => (
                      ...
                      >
                        <td style={{ padding: "10px 14px", color: "#6B6860" }}>
                        ...
                        >
                          {formatNaira(d.amount as number)}
                        </td>
                        ...
                        >
                          {d.transaction_reference as string}
                        </td>
                      </tr>
                    ))}
                  </tbody>
